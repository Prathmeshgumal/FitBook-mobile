...
const GYM_TYPES = ['Weight Training', 'Yoga', 'CrossFit', 'Zumba', 'Mixed Martial Arts', 'Other'];
    ...
    <View style={styles.root}>
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={styles.flex}>
        <Animated.View style={[styles.card, { transform: [{ translateY: cardAnim }] }]}>
          ...
          >
            <Field label="Gym Name *" value={name} onChangeText={setName}
              placeholder="e.g. FitBook Gym" focused={focused} field="name" setFocused={setFocused} />

            {/* Gym Type selector */}
            <View style={styles.fieldWrap}>
              <Text style={styles.label}>Gym Type *</Text>
              <TouchableOpacity
                style={[styles.input, styles.selectRow, focused === 'type' && styles.inputFocused]}
                onPress={() => setShowTypeDropdown(v => !v)}
                activeOpacity={0.8}
              >
                <Text style={gymType ? styles.selectText : styles.placeholderText}>
                  {gymType || 'Select gym type'}
                </Text>
                <Ionicons name={showTypeDropdown ? 'chevron-up' : 'chevron-down'} size={16} color="#8890A8" />
              </TouchableOpacity>
              {showTypeDropdown && (
                <View style={styles.dropdown}>
                  {GYM_TYPES.map(t => (
                    <TouchableOpacity key={t} style={styles.dropdownItem}
                      onPress={() => { setGymType(t); setShowTypeDropdown(false); }}>
                      <Text style={[styles.dropdownText, gymType === t && styles.dropdownTextActive]}>{t}</Text>
                    </TouchableOpacity>
                  ))}
                </View>
              )}
            ...
            <Field label="Phone Number *" value={phone} onChangeText={setPhone}
              placeholder="10–15 digits" keyboardType="phone-pad"
              focused={focused} field="phone" setFocused={setFocused} />
            <Field label="Address *" value={address} onChangeText={setAddress}
              placeholder="Street address"
              focused={focused} field="address" setFocused={setFocused} />
            <View style={styles.row}>
              ...
              <View style={[styles.fieldWrap, styles.flex]}>
                ...
                <TextInput
                  ...
                  value={state} onChangeText={setState} placeholder="State"
                  placeholderTextColor={PLACEHOLDER}
                  onFocus={() => setFocused('state')} onBlur={() => setFocused(null)} />
              </View>
            </View>
            <Field label="Opening Hours" value={openingHours} onChangeText={setOpeningHours}
              placeholder="e.g. 6:00 AM – 10:00 PM"
              focused={focused} field="hours" setFocused={setFocused} />
...
const Field = ({ label, value, onChangeText, placeholder, keyboardType, focused, field, setFocused }: any) => (
  <View style={styles.fieldWrap}>
...
const styles = StyleSheet.create({
  ...
  dropdown: { backgroundColor: '#F8F9FC', borderRadius: 8, marginTop: 4, borderWidth: 1, borderColor: DIVIDER },
  dropdownItem: { paddingHorizontal: 16, paddingVertical: 12, borderBottomWidth: 1, borderBottomColor: DIVIDER },
