...
const MembersListScreen = ({ navigation }: MembersListScreenProps) => {
  ...
  const loadMembers = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const data = await listMembers(gym.id, {
  ...
  const closeSidebar = () => {
    Animated.timing(sidebarAnim, { toValue: 300, duration: 280, useNativeDriver: true }).start(() => setShowSidebar(false));
  };
  ...
  return (
    <View style={[styles.root, { paddingTop: insets.top }]}>
      ...
      <View style={styles.searchWrap}>
        <Ionicons name="search-outline" size={18} color="#8890A8" style={styles.searchIcon} />
        <TextInput
      ...
      <View style={styles.filterRow}>
        {/* Gender filter */}
        {['', 'male', 'female'].map(g => (
          <TouchableOpacity key={g} style={[styles.filterChip, gender === g && styles.filterChipActive]}
            onPress={() => setGender(g)}>
        ...
        {/* Plan filter */}
        <View>
          <TouchableOpacity style={[styles.filterChip, planId !== undefined && styles.filterChipActive]}
            onPress={() => setShowPlanFilter(v => !v)}>
            <Text style={[styles.filterChipText, planId !== undefined && styles.filterChipTextActive]}>
              {planId ? plans.find(p => p.id === planId)?.name ?? 'Plan' : 'All Plans'}
            </Text>
            <Ionicons name="chevron-down" size={12} color={planId !== undefined ? '#FFFFFF' : '#555'} style={{ marginLeft: 4 }} />
          </TouchableOpacity>
          {showPlanFilter && (
            <View style={styles.dropdownAbsolute}>
              <TouchableOpacity style={styles.dropdownItem} onPress={() => { setPlanId(undefined); setShowPlanFilter(false); }}>
                <Text style={styles.dropdownText}>All Plans</Text>
              </TouchableOpacity>
              {plans.map(p => (
                <TouchableOpacity key={p.id} style={styles.dropdownItem}
                  onPress={() => { setPlanId(p.id); setShowPlanFilter(false); }}>
                  <Text style={[styles.dropdownText, planId === p.id && styles.dropdownTextActive]}>{p.name}</Text>
                </TouchableOpacity>
              ))}
            </View>
          )}
        ...
        {/* Sort */}
        <View>
          <TouchableOpacity style={styles.filterChip} onPress={() => setShowSortFilter(v => !v)}>
            <Ionicons name="swap-vertical" size={13} color="#555" />
            <Text style={styles.filterChipText}>{sort === 'newest' ? 'Newest' : 'Oldest'}</Text>
          </TouchableOpacity>
          {showSortFilter && (
            <View style={styles.dropdownAbsolute}>
              {(['newest', 'oldest'] as const).map(s => (
                <TouchableOpacity key={s} style={styles.dropdownItem}
                  onPress={() => { setSort(s); setShowSortFilter(false); }}>
                  <Text style={[styles.dropdownText, sort === s && styles.dropdownTextActive]}>
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <View style={styles.emptyState}>
              ...
              <Text style={styles.emptySub}>Add your first member to get started</Text>
          ...
          >
            <TouchableOpacity activeOpacity={1}>
              ...
              <View style={styles.sidebarSection}>
                ...
                <TouchableOpacity style={styles.sidebarRow}
                  onPress={() => {
                    closeSidebar();
                    // Navigate to payment methods config — placeholder for now
                    Alert.alert('Payment Methods', 'Configure payment methods in the Settings tab (coming soon).');
                  }}>
              ...
              <View style={styles.sidebarSection}>
                ...
                <TouchableOpacity style={styles.sidebarRow}
                  onPress={() => {
                    closeSidebar();
                    Alert.alert('Edit Gym', 'Gym profile editing coming in Settings tab.');
                  }}>
...
const styles = StyleSheet.create({
  ...
  filterRow: { flexDirection: 'row', paddingHorizontal: 16, marginBottom: 8, gap: 8, flexWrap: 'wrap', zIndex: 10 },
  filterChip: {
    flexDirection: 'row', alignItems: 'center',
  ...
  filterChipText: { fontSize: 12, color: 'rgba(255,255,255,0.8)', fontWeight: '500' },
  filterChipTextActive: { color: BG, fontWeight: '700' },
  dropdownAbsolute: {
    position: 'absolute', top: 32, left: 0, zIndex: 100,
    backgroundColor: '#FFFFFF', borderRadius: 8, minWidth: 140,
    shadowColor: '#000', shadowOpacity: 0.15, shadowRadius: 8, shadowOffset: { width: 0, height: 4 },
    elevation: 8,
  },
  dropdownItem: { paddingHorizontal: 16, paddingVertical: 10, borderBottomWidth: 1, borderBottomColor: '#F0F0F0' },
  ...
  memberInfo: { flex: 1 },
  memberTopRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 3 },
  memberName: { fontSize: 15, fontWeight: '700', color: '#1A1A1A' },
  memberSub: { fontSize: 12, color: '#666', marginBottom: 6 },
  pillRow: { flexDirection: 'row', gap: 6 },
  pillBatch: { backgroundColor: '#E8EAF6', borderRadius: 20, paddingHorizontal: 8, paddingVertical: 2 },
  pillPlan: { backgroundColor: '#E8F5E9', borderRadius: 20, paddingHorizontal: 8, paddingVertical: 2 },
  pillText: { fontSize: 10, color: '#333', fontWeight: '500' },
  dueBadge: { backgroundColor: '#FFF3E0', borderRadius: 20, paddingHorizontal: 8, paddingVertical: 2 },
  dueBadgeText: { fontSize: 10, color: '#E65100', fontWeight: '700' },
  activeBadge: { backgroundColor: '#E8F5E9', borderRadius: 20, paddingHorizontal: 8, paddingVertical: 2 },
  ...
  emptySub: { fontSize: 14, color: '#8890A8', textAlign: 'center', marginBottom: 24 },
  emptyBtn: { backgroundColor: '#FFFFFF', borderRadius: 24, paddingHorizontal: 24, paddingVertical: 12 },
  emptyBtnText: { color: BG, fontWeight: '700', fontSize: 14 },
  fab: {
    position: 'absolute', right: 20,
    backgroundColor: BG, borderRadius: 30, height: 52,
    paddingHorizontal: 20, flexDirection: 'row', alignItems: 'center', gap: 8,
  ...
  sidebar: {
    ...
    backgroundColor: '#FFFFFF', paddingTop: 56, paddingHorizontal: 24, zIndex: 51,
  },
  sidebarTitle: { fontSize: 20, fontWeight: '800', color: BG, marginBottom: 24 },
  sidebarSection: { marginBottom: 24 },
  sidebarSectionTitle: { fontSize: 11, fontWeight: '700', color: '#AAAAAA', letterSpacing: 1, marginBottom: 10, textTransform: 'uppercase' },
  sidebarRow: { flexDirection: 'row', alignItems: 'center', gap: 12, paddingVertical: 12, borderBottomWidth: 1, borderBottomColor: '#F0F0F0' },
  sidebarRowText: { flex: 1, fontSize: 14, color: '#1A1A1A', fontWeight: '500' },
});
