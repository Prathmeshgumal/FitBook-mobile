...
import { getMemberDetail, updateMember, addPayment, deletePayment, type MemberDetail, type Membership } from '../../api/members';
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scroll}>
        ...
        <View style={styles.profileCard}>
          <View style={styles.profileTop}>
            ...
            <View style={styles.profileInfo}>
              ...
              <Text style={styles.profileSub}>M ID: {member.membership_id}</Text>
              <Text style={styles.profileSub}>Mobile: +91-{member.phone}</Text>
              {member.gender && <Text style={styles.profileSub}>Gender: {member.gender.charAt(0).toUpperCase() + member.gender.slice(1)}</Text>}
            </View>
          </View>
          {member.address && <Text style={styles.profileAddress}>{member.address}</Text>}
          {member.notes && <Text style={styles.profileNotes}>{member.notes}</Text>}
        </View>
        ...
        <View style={styles.actionBar}>
          {[
            ...
            { icon: 'logo-whatsapp', label: 'WhatsApp', onPress: handleWhatsApp },
            { icon: 'refresh-outline', label: 'Renew Plan', onPress: () => Alert.alert('Renew', 'Coming soon — use the plans section below.') },
            { icon: member.is_blocked ? 'lock-open-outline' : 'ban-outline', label: member.is_blocked ? 'Unblock' : 'Block', onPress: handleBlock },
          ].map(action => (
            <TouchableOpacity key={action.label} style={styles.actionItem} onPress={action.onPress}>
...
const MembershipCard = ({
  ms, memberId, onRefresh, showPaymentForm, setShowPaymentForm, payAmount, setPayAmount,
}: {
  ms: Membership;
  memberId: number;
  onRefresh: () => void;
  showPaymentForm: number | null;
  setShowPaymentForm: (id: number | null) => void;
  payAmount: string;
  setPayAmount: (v: string) => void;
}) => {
  const due = parseFloat(ms.full_amount) - parseFloat(ms.discount) - parseFloat(ms.total_paid);
    ...
    <View style={styles.planCard}>
      ...
      <View style={styles.planRow}>
        <View style={styles.planCell}>
          ...
          <Text style={styles.planCellValue}>{formatDate(ms.purchase_date)}</Text>
        </View>
        <View style={styles.planCell}>
          <Text style={styles.planCellLabel}>Expiry Date</Text>
          <Text style={[styles.planCellValue, { color: isExpired(ms.expiry_date) ? '#E53935' : '#4CAF50' }]}>
      ...
      <View style={styles.planRow}>
        ...
        <View style={styles.planCell}>
          ...
          <Text style={[styles.planCellValue, { color: due > 0 ? '#E53935' : '#4CAF50' }]}>
            ₹{due.toFixed(2)}
          </Text>
        </View>
      </View>
      ...
      {isShowingForm ? (
        <View style={styles.payFormRow}>
          ...
          />
          <TouchableOpacity style={styles.payConfirmBtn} onPress={handleAddPayment}>
        ...
        <TouchableOpacity style={styles.addPaymentBtn} onPress={() => setShowPaymentForm(ms.id)}>
...
const styles = StyleSheet.create({
  ...
  profileCard: { backgroundColor: '#FFFFFF', borderRadius: 14, padding: 16, marginBottom: 12 },
  profileTop: { flexDirection: 'row', gap: 14, marginBottom: 8 },
  profileInfo: { flex: 1, justifyContent: 'center' },
  profileName: { fontSize: 17, fontWeight: '800', color: '#1A1A1A', marginBottom: 3 },
  ...
  actionBar: { backgroundColor: '#FFFFFF', borderRadius: 14, flexDirection: 'row', justifyContent: 'space-around', paddingVertical: 14, marginBottom: 16 },
  actionItem: { alignItems: 'center', gap: 6 },
  actionIcon: { width: 44, height: 44, borderRadius: 22, backgroundColor: '#F0F2F5', alignItems: 'center', justifyContent: 'center' },
  ...
  emptyCard: { backgroundColor: '#FFFFFF', borderRadius: 14, padding: 20, alignItems: 'center', marginBottom: 12 },
  emptyText: { fontSize: 14, color: '#AAAAAA' },

  planCard: { backgroundColor: '#FFFFFF', borderRadius: 14, padding: 16, marginBottom: 12 },
  planName: { fontSize: 16, fontWeight: '700', color: '#1A1A1A', marginBottom: 12 },
  planRow: { flexDirection: 'row', marginBottom: 10 },
  planCell: { flex: 1 },
  planCellLabel: { fontSize: 11, color: '#AAAAAA', fontWeight: '600', marginBottom: 3, textTransform: 'uppercase' },
  planCellValue: { fontSize: 14, color: '#1A1A1A', fontWeight: '600' },
  planComments: { fontSize: 13, color: '#666', marginTop: 6, fontStyle: 'italic' },

  paymentList: { marginTop: 12, borderTopWidth: 1, borderTopColor: '#F0F0F0', paddingTop: 10 },
  paymentListLabel: { fontSize: 11, color: '#AAAAAA', fontWeight: '600', textTransform: 'uppercase', marginBottom: 6 },
  paymentRow: { flexDirection: 'row', alignItems: 'center', gap: 12, marginBottom: 6 },
  paymentDate: { flex: 1, fontSize: 13, color: '#555' },
  paymentAmount: { fontSize: 13, fontWeight: '700', color: '#1A1A1A' },

  addPaymentBtn: { flexDirection: 'row', alignItems: 'center', gap: 8, justifyContent: 'center', borderWidth: 1, borderColor: BG, borderRadius: 8, paddingVertical: 10, marginTop: 12 },
  addPaymentText: { fontSize: 14, color: BG, fontWeight: '600' },

  payFormRow: { flexDirection: 'row', alignItems: 'center', gap: 10, marginTop: 12 },
  payInput: { flex: 1, borderBottomWidth: 1, borderBottomColor: '#E0E0E0', fontSize: 15, color: '#1A1A1A', paddingBottom: 6 },
  payConfirmBtn: { backgroundColor: BG, width: 36, height: 36, borderRadius: 18, alignItems: 'center', justifyContent: 'center' },
});
